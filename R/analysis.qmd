...
After identifying 4 clusters using hierarchical clustering, we apply a K-means algorithm to refine this partition. The goal is to improve the BSS/TSS ratio, as although Ward's algorithm produced a 4-cluster classification, there is no guarantee that it represents the optimal 4 clusters' partition.
...
```{r mca-quality-var}
#| label: tbl-cos2-var
#| tbl-cap: "Variable representation quality (cos²) on the first factorial plane"
cos2_var_mca <- round(sort(rowSums(mca_result$var$cos2[, 1:2])), digits = 3)
knitr::kable(round(cos2_var_mca,2))
```

Based on @tbl-cos2-var, we see that the modality `BMI.Category_Normal` is the best represented on the first principal plane, with a $\cos^2$  of `r cos2_var_mca["BMI.Category_Normal"]`, while `BMI.Category_Obese` is the least well represented, with a $\cos^2$ of `r cos2_var_mca["BMI.Category_Obese"]`. The categories related to sleep and the blood pressure profiles are also well represented, with $\cos^2$ values ≥ 0.669. In contrast, the sleep profiles, particularly `Below 6.5` and `6.5 to 7.5`, are less well represented, with $\cos^2$ values of 0.284 and 0.444, respectively.
...
```{r}
#| label: tbl-var-contrib
#| tbl-cap: "Variable contributions to the first factorial plane"
cos2_var_contrib <- mca_result$var$contrib[,c(1,2)]
knitr::kable(round(cos2_var_contrib,2))
```

The contributions of the different modalities to the first two dimensions of the MCA reveal interesting patterns. `BMI.Category_Overweight` (17.575) and `BMI.Category_Normal` (13.819) show notable contributions to Dimension 1, indicating that they are well differentiated along this axis. The blood pressure profiles `BP_Profile_Hypertension` (10.399) and `BP_Profile_Normal` (15.021) also contribute substantially to Dimension 1. On the other hand, `Insomnia` (22.971) and `Sleep Apnea` (28.959) are strongly represented on Dimension 2, suggesting a strong relationship with this dimension. The categories `Above 7.5` (23.655) and `6.5 to 7.5` (23.479) also stand out on Dimension 2, while `Below 6.5` (7.139) shows a lower contribution to both dimensions (see @tbl-var-contrib).
...
#| tbl-cap: "Summary Statistics of quantitative variables"
